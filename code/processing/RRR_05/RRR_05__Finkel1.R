...
convert_raw_data_Finkel1_fun <- function(data, variable_info = NULL, output_folder){
  ...
  all_Data_Collection_Sites_fun <- function(name){
    ...
    # Exit Forgiveness #
    Exit.raw <- with(Data_Collection_Site,data.frame(Q2.2_1,Q2.3_3,Q2.4_1,Q2.5_2,Q2.6_2,Q2.7_4,Q2.8_2,Q2.9_4,Q2.10_1,Q2.11_4,Q2.12_3,Q2.13_4)) # read Exit items
    ...
    return(
      cbind(
        data.frame(
          Data_Collection_Site = rep(name, length(Exit)),
          DV = Exit,
          group = Condition
        ),
        Exit.data)
