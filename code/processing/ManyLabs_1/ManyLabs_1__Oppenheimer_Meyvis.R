...
convert_raw_data_Oppenheimer_Meyvis_fun <- function(data, variable_info = NULL, output_folder){

  ## create final data set
  d <- data.frame(MultiLab = rep("ML1", nrow(data)),
                  MASC = rep("Oppenheimer_Meyvis", nrow(data)),
                  Data_Collection_Site = data$sample,
                  DV = data$sunkDV,
                  Group = as.character(data$sunkgroup)
  )

  ## treatment and control group as 1 and 0
  d$Group[d$Group == "paid"] <- 1
  d$Group[d$Group == "free"] <- 0
