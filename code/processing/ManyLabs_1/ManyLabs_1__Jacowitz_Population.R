...
convert_raw_data_Jacowitz_Population_fun <- function(data, variable_info = NULL, output_folder){

  ## full data set
  d <- data
  ## create subset for this Data_Collection_Site
  d <- data.frame(MultiLab = rep("ML1", nrow(d)),
                  MASC = rep("Jacowitz_Population", nrow(d)),
                  Data_Collection_Site = d$sample,
                  DV = d$anchoring2,
                  Group = as.character(d$anch2group)
  )
