...
convert_raw_data_Jacowitz_Distance_fun <- function(data, variable_info = NULL, output_folder){

  ## full data set
  d <- data
  ## create subset for this Data_Collection_Site
  d <- data.frame(MultiLab = rep("ML1", nrow(d)),
                  MASC = rep("Jacowitz_Distance", nrow(d)),
                  Data_Collection_Site = d$sample,
                  DV = d$anchoring1,
                  Group = as.character(d$anch1group)
                  # , Meter = ifelse(is.na(ML1_data$anchoring1akm), 0, 1) + ifelse(is.na(ML1_data$anchoring1bkm), 0, 1)
  )
