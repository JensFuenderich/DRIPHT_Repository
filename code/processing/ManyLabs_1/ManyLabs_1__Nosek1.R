...
convert_raw_data_Nosek1_fun <- function(data, variable_info = NULL, output_folder){

  ## full data set
  d <- data
  ## create subset for this Data_Collection_Site
  d <- data.frame(MultiLab = rep("ML1", nrow(d)),
                  MASC = rep("Nosek1", nrow(d)),
                  Data_Collection_Site = d$sample,
                  DV = d$d_art,
                  DV_Item_1 = d$d_art1,
                  DV_Item_2 = d$d_art1,
                  Group = as.character(d$partgender)
  )

  ## treatment (women) and control (men) group as 1 and 0
  d <- d %>% dplyr::filter(Group != "1")
  d$Group[d$Group == "female"] <- "1"
  d$Group[d$Group == "male"] <- "0"
