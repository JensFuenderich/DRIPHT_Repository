...
convert_raw_data_Lorge_fun <- function(data, variable_info = NULL, output_folder){

  ## full data set
  d <- data
  ## create subset for this Data_Collection_Site
  d <- data.frame(MultiLab = rep("ML1", nrow(d)),
                  MASC = rep("Lorge", nrow(d)),
                  Data_Collection_Site = d$sample,
                  DV = d$quote,
                  Group = as.character(d$quoteGroup)
  )

  ## treatment and control group as 1 and 0
  d$Group[d$Group == "liked source"] <- 1
  d$Group[d$Group == "disliked source"] <- 0
