...
### MASC: Reluctance to tempt fate (Risen & Gilovich, 2008)
...
# This version uses a subset that is reduced to only undergraduate students across all samples, to achieve a close(r) replication.

convert_raw_data_Risen_CR_fun <- function(data, variable_info = NULL, output_folder){

  # # print variables for Risen
  # Risen_vars <- variable_info[variable_info$study.analysis == "Risen.3",]
  # Risen_vars <- eval(parse(text=Risen_vars$study.vars[1]))
  # print(Risen_vars)

  ## DV_Item_1
  # select dependent variable (/items) of interest
  # univariate selection
  dv_item_1_treatment <- data$rise1.3
  dv_item_1_control <- data$rise2.3
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Risen_CR", length(dv_item_1)),
                                           Data_Collection_Site = Data_Collection_Site,
                                           DV_Item_1 = dv_item_1,
                                           Group = group,
                                           SubjectPool = data$SubjectPool
  )
  ## excluding cases to reduce to primary analysis (in accordance with ML2_masteRkey)
  # "For the key confirmatory test, the original authors suggested that the sample should include only undergraduate students"
  # ML2_masteRkey[ML2_masteRkey$study.analysis == "Risen.1",]$study.sites.include
  Item_Level_before_cleaning <- subset(Item_Level_before_cleaning, Item_Level_before_cleaning$SubjectPool == "Yes")

  ## deleting subject pool column
  Item_Level_before_cleaning$SubjectPool <- NULL

  # apply data cleaning
  # including cases according to ML2_masteRkey
  #ML2_masteRkey[ML2_masteRkey$study.analysis == "Risen.1",]$study.cases.include
