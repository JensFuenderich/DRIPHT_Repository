...
### MASC: Perceived intentionality for side effects (Knobe, 2003)
...
convert_raw_data_Knobe_fun <- function(data, variable_info = NULL, output_folder){

  # # print variables for Knobe
  # Knobe_vars <- variable_info[variable_info$study.analysis == "Knobe.1",]
  # Knobe_vars <- eval(parse(text=Knobe_vars$study.vars[1]))
  # print(Knobe_vars)

  ## DV_Item_1
  # select dependent variable (/items) of interest
  # univariate selection
  dv_item_1_treatment <- data$knob2.3
  dv_item_1_control <- data$knob1.3
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Knobe", length(dv_item_1)),
  ...
  #ML2_masteRkey[ML2_masteRkey$study.analysis == "Knobe.1",]$study.cases.include
