...
### MASC: Consumerism undermines trust (Bauer, Wilkie, Kim, & Bodenhausen, 2012)
...
###
### IN CASE OF DEVIATION FROM EXCEL: there is a treatment column for this
### data set but the exclusion in ML2MasteRkey is defined through the DV itself...
### i went with the treatment column for now.
### I could(/should?) change that to what's specified in ML2MasteRkey.
###

convert_raw_data_Bauer_fun <- function(data, variable_info = NULL, output_folder){

  # # print variables for Bauer condition
  # Bauer_vars <- ML2_masteRkey[ML2_masteRkey$study.analysis == "Bauer.1",]
  # Bauer_vars <- eval(parse(text=Bauer_vars$study.vars[1]))
  # print(Bauer_vars)

  ## DV_Item_1
  # select dependent variable (/items) of interest
  # univariate selection
  dv_item_1 <- data$baudv.4


  ## Group
  # select independent variable (/items) of interest
  treatment_group <- data$bau2.1
  # change all NA to 0
  treatment_group <- replace(treatment_group, treatment_group > 0, 1)
  # selecting all control group variables
  control_group <- data$bau1.1
  # change all NA to 0
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Bauer", length(dv_item_1)),
  ...
  #ML2_masteRkey[ML2_masteRkey$study.analysis == "Bauer.1",]$study.cases.include
  Item_Level <- Item_Level_before_cleaning %>%
    dplyr::filter(DV_Item_1 < 8) %>%
