...
### MASC: Disgust sensitivity predicts homophobia (Inbar, Pizarro, Knobe, & Bloom, 2009)
...
# The primary Data_Collection_Site of Inbar et al. is a correlational design. We replicated the difference in intentionality ratings between the SameKiss and DiffKiss condition.

convert_raw_data_Inbar_fun <- function(data, variable_info = NULL, output_folder){

  # # print variables for Hsee condition
  # Inbar_vars <- ML2_masteRkey[ML2_masteRkey$study.analysis == "Inbar.1a",]
  # Inbar_vars <- eval(parse(text=Inbar_vars$study.vars[1]))
  # print(Inbar_vars)

  ## DV_Item_1
  # select dependent variable (/items) of interest
  # univariate selection
  dv_item_1_treatment <- data$inba1.3
  dv_item_1_control <- data$inba2.3
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Inbar", length(dv_item_1)),
  ...
  #ML2_masteRkey[ML2_masteRkey$study.analysis == "Inbar.1",]$study.cases.include
