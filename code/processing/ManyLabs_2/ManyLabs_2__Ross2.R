...
### MASC: False consensus: traffic-ticket scenario (Ross et al., 1977)
...
convert_raw_data_Ross2_fun <- function(data, variable_info = NULL, output_folder){

  # # print variables for Ross1
  # Ross2_vars <- ML2_masteRkey[ML2_masteRkey$study.analysis == "Ross.2",]
  # Ross2_vars <- eval(parse(text=Ross2_vars$study.vars[1]))
  # print(Ross2_vars)

  ## DV_Item_1
  # select dependent variable (/items) of interest
  # univariate selection
  dv_item_1 <- data$ross.s2.1_1_TEXT

  ## Group
  # select independent variable (/items) of interest
  group <- data$ross.s2.2
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Ross2", length(dv_item_1)),
  ...
  #ML2_masteRkey[ML2_masteRkey$study.analysis == "Ross.2",]$study.cases.include
