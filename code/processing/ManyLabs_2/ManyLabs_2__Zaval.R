...
convert_raw_data_Zaval_fun <- function(data, variable_info = NULL, output_folder){
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Zaval", length(dv_item_1)),
  ...
  # Item_Level <- Item_Level %>% dplyr::filter(Language == "English") # this line is only used in the non-CR version
