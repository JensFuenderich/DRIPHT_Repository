...
### MASC: Moral typecasting (Gray & Wegner, 2009)
...
convert_raw_data_Gray_fun <- function(data, variable_info = NULL, output_folder){

  # # print variables for Gray condition
  # Gray_vars <- ML2_masteRkey[ML2_masteRkey$study.analysis == "Gray.1",]
  # Gray_vars <- eval(parse(text=Gray_vars$study.vars[1]))
  # print(Gray_vars)

  ## DV_Item_1
  # select dependent variable (/items) of interest
  # univariate selection
  dv_item_1_treatment <- data$gray1.2
  dv_item_1_control <- data$gray2.2
  ...
  Item_Level_before_cleaning <- data.frame(MultiLab = rep("ML2", length(dv_item_1)),
                                           MASC = rep("Gray", length(dv_item_1)),
  ...
  #ML2_masteRkey[ML2_masteRkey$study.analysis == "Gray.1",]$study.cases.include
